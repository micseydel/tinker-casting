...
object BubbleSortCell {
  ...
  sealed trait MessageToRespondTo extends Message {
    def replyTo: SpiritRef[NonEmptyList[BubbleSortCellWrapper]]
  }

  final case class GetDownstreamListContents(replyTo: SpiritRef[NonEmptyList[BubbleSortCellWrapper]]) extends MessageToRespondTo

  final case class RequestSortedContents(replyTo: SpiritRef[NonEmptyList[BubbleSortCellWrapper]]) extends MessageToRespondTo

  private case class NewNeighbor(leftOrRight: Either[Option[BubbleSortCellWrapper], Option[BubbleSortCellWrapper]],
                                 retainSort: Boolean = false,
                                 maybeExternalReplyTo: Option[SpiritRef[NonEmptyList[BubbleSortCellWrapper]]] = None
                                ) extends Message

  private case class ReceiveSortedDownstream(sortedDownstream: NonEmptyList[BubbleSortCellWrapper]) extends Message
  ...
  private def setup(id: Int, index: Int, noteName: String, value: Int, remaining: List[(Int, String, Int)], leftNeighbor: Option[BubbleSortCellWrapper])(implicit Tinker: EnhancedTinker[SelfSortingArrayCentralCast]): Ability[Message] = NoteMakingTinkerer(noteName, TinkerColor.random(), "📵") { (context, noteRef) =>
    ...
    remaining match {
      ...
      case (nextId, nextNoteName, nextValue) :: tail =>
        val rightNeighbor = context.cast(BubbleSortCell.setup(nextId, index+1, nextNoteName, nextValue, tail, leftNeighbor = Some(self)), nextNoteName.replace(" ", "_").replace("(", "").replace(")", ""))
        waiting(CellState(id, index, value, leftNeighbor, Some(CellWrapper(nextId, nextValue, nextNoteName, rightNeighbor))))
    }
  }

  private def waiting(state: CellState)(implicit Tinker: EnhancedTinker[SelfSortingArrayCentralCast], self: BubbleSortCellWrapper, noteRef: NoteRef): Ability[Message] = {
  ...
  private def awaitingAggregation(state: CellState, replyTo: SpiritRef[NonEmptyList[BubbleSortCellWrapper]])(implicit Tinker: EnhancedTinker[SelfSortingArrayCentralCast], self: BubbleSortCellWrapper, noteRef: NoteRef): Ability[Message] = Tinker.setup { context =>
    implicit val tc: TinkerContext[?] = context
    Tinker.userExtension.probe !! state.probe
    noteRef.setRaw(
      s"""---
  ...
  private def awaitingSortedDownstream(state: CellState, maybeCallerReplyTo: Option[SpiritRef[NonEmptyList[BubbleSortCellWrapper]]])(implicit Tinker: EnhancedTinker[SelfSortingArrayCentralCast], self: BubbleSortCellWrapper, noteRef: NoteRef): Ability[Message] = Tinker.setup { context =>
    implicit val tc: TinkerContext[?] = context
    Tinker.userExtension.probe !! state.probe
  ...
  private def sorted(state: CellState, sortedDownstream: List[BubbleSortCellWrapper])(implicit Tinker: EnhancedTinker[SelfSortingArrayCentralCast], self: BubbleSortCellWrapper, noteRef: NoteRef): Ability[Message] = Tinker.setup { context =>
    implicit val tc: TinkerContext[?] = context
    Tinker.userExtension.probe !! state.probe
  ...
  private def findNewDownstreamNeighbors(value: Int, sortedDownstream: NonEmptyList[BubbleSortCellWrapper]): (BubbleSortCellWrapper, List[BubbleSortCellWrapper]) = {
    @tailrec
    def helper(remaining: NonEmptyList[BubbleSortCellWrapper]): (BubbleSortCellWrapper, List[BubbleSortCellWrapper]) = {
      remaining match {
  ...
  private case class CellState(id: Int, index: Int, value: Int, maybeLeftNeighbor: Option[BubbleSortCellWrapper], maybeRightNeighbor: Option[BubbleSortCellWrapper]) {
    def withUpdatedRight(newRight:  Option[BubbleSortCellWrapper]): CellState = this.copy(maybeRightNeighbor = newRight)
    def withUpdatedLeft(newLeft:  Option[BubbleSortCellWrapper]): CellState = this.copy(maybeLeftNeighbor = newLeft)
