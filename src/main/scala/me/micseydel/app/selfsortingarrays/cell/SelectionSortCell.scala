...
object SelectionSortCell {
  ...
  type SelectionSortCellWrapper = CellWrapper[Message]
  ...
  sealed trait MessageToRespondTo extends Message {
    def replyTo: SpiritRef[NonEmptyList[SelectionSortCellWrapper]]
  }

  final case class GetDownstreamListContents(replyTo: SpiritRef[NonEmptyList[SelectionSortCellWrapper]]) extends MessageToRespondTo

  final case class RequestSortedContents(replyTo: SpiritRef[NonEmptyList[SelectionSortCellWrapper]]) extends MessageToRespondTo

  private case class NewNeighbor(leftOrRight: Either[Option[SelectionSortCellWrapper], Option[SelectionSortCellWrapper]],
                                 retainSort: Boolean = false,
                                 maybeExternalReplyTo: Option[SpiritRef[NonEmptyList[SelectionSortCellWrapper]]] = None
                                ) extends Message

  private case class ReceiveSortedDownstream(sortedDownstream: NonEmptyList[SelectionSortCellWrapper]) extends Message
  ...
  private def setup(id: Int, index: Int, noteName: String, value: Int, remaining: List[(Int, String, Int)], leftNeighbor: Option[SelectionSortCellWrapper])(implicit Tinker: EnhancedTinker[SelfSortingArrayCentralCast]): Ability[Message] = NoteMakingTinkerer(noteName, TinkerColor.random(), "📵") { (context, noteRef) =>
    implicit val self: SelectionSortCellWrapper = CellWrapper(id, value, noteName, context.self)
    implicit val nr: NoteRef = noteRef
    remaining match {
      case Nil =>
        waiting(CellState(id, index, value, leftNeighbor, None))
      case (nextId, nextNoteName, nextValue) :: tail =>
        val rightNeighbor = context.cast(SelectionSortCell.setup(nextId, index+1, nextNoteName, nextValue, tail, leftNeighbor = Some(self)), nextNoteName.replace(" ", "_").replace("(", "").replace(")", ""))
        waiting(CellState(id, index, value, leftNeighbor, Some(CellWrapper(nextId, nextValue, nextNoteName, rightNeighbor))))
    }
  }

  private def waiting(state: CellState)(implicit Tinker: EnhancedTinker[SelfSortingArrayCentralCast], self: SelectionSortCellWrapper, noteRef: NoteRef): Ability[Message] = {
  ...
  private def awaitingAggregation(state: CellState, replyTo: SpiritRef[NonEmptyList[SelectionSortCellWrapper]])(implicit Tinker: EnhancedTinker[SelfSortingArrayCentralCast], self: SelectionSortCellWrapper, noteRef: NoteRef): Ability[Message] = Tinker.setup { context =>
    implicit val tc: TinkerContext[?] = context
    Tinker.userExtension.probe !! state.probe
    noteRef.setRaw(
      s"""---
  ...
  private def awaitingSortedDownstream(state: CellState, maybeCallerReplyTo: Option[SpiritRef[NonEmptyList[SelectionSortCellWrapper]]])(implicit Tinker: EnhancedTinker[SelfSortingArrayCentralCast], self: SelectionSortCellWrapper, noteRef: NoteRef): Ability[Message] = Tinker.setup { context =>
    implicit val tc: TinkerContext[?] = context
    Tinker.userExtension.probe !! state.probe
  ...
  private def sorted(state: CellState, sortedDownstream: List[SelectionSortCellWrapper])(implicit Tinker: EnhancedTinker[SelfSortingArrayCentralCast], self: SelectionSortCellWrapper, noteRef: NoteRef): Ability[Message] = Tinker.setup { context =>
    implicit val tc: TinkerContext[?] = context
    Tinker.userExtension.probe !! state.probe
  ...
  private def findNewDownstreamNeighbors(value: Int, sortedDownstream: NonEmptyList[SelectionSortCellWrapper]): (SelectionSortCellWrapper, List[SelectionSortCellWrapper]) = {
    @tailrec
    def helper(remaining: NonEmptyList[SelectionSortCellWrapper]): (SelectionSortCellWrapper, List[SelectionSortCellWrapper]) = {
      remaining match {
  ...
  private case class CellState(id: Int, index: Int, value: Int, maybeLeftNeighbor: Option[SelectionSortCellWrapper], maybeRightNeighbor: Option[SelectionSortCellWrapper]) {
    def withUpdatedRight(newRight:  Option[SelectionSortCellWrapper]): CellState = this.copy(maybeRightNeighbor = newRight)
    def withUpdatedLeft(newLeft:  Option[SelectionSortCellWrapper]): CellState = this.copy(maybeLeftNeighbor = newLeft)
