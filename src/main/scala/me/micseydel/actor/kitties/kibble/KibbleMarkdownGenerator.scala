...
object KibbleMarkdownGenerator {
  def apply(allEvents: List[Event])(implicit log: Logger): String = {
...
case class WithinDayState(day: LocalDate,
   ...
   */
  def integrate(event: Event, latestMeasurements: LatestMeasurements)(implicit log: Logger): (WithinDayState, LatestMeasurements) = {
    event match {
