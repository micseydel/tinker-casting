...
from pa_support import runner, get_config_from_env


NOTE_NAME = "Kokoro PythonActor Testing"
...
class PythonActor:
    def __init__(self, note_dir, note_path):
    ...
    def setup(self, mqtt_q):
        ...
        mqtt_q.subscribe("test/PythonActor/input")
    ...
    def run_kokoro_with_frontmatter(self):
        with open(self.note_path) as f:
            ...
            for i, (gs, ps, audio) in enumerate(generator):
                ...
                filename = f"{now}_{voice}_{i}.wav"
                wav_path = os.path.join(output_dir, filename)
                logging.info(f"i, gs, ps = {i}, {gs}, {ps}")
                logging.info(f"About to write {voice} output to {wav_path}")
                sf.write(wav_path, audio, 24000)
                if play_after_creation:
                    self.mqtt_q.publish("[[Sound Player]]", wav_path)
            took = time() - start
            logging.info(f"took {took}s")
...
if __name__ == "__main__":
    ...
    note_dir = sys.argv[1]
    note_path = os.path.join(note_dir, f"{NOTE_NAME}.md")
    runner(get_config_from_env(), note_dir, note_path, PythonActor)
