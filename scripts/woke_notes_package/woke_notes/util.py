...
def seconds_until(later: datetime) -> float | int:
    return (later - datetime.now(timezone.utc)).total_seconds()


# generated with claude-opus-4-6 on 2026-07-13 to avoid pytz
def next_occurrence(time_str: str, now: datetime = None) -> datetime:
    """
...
if __name__ == '__main__':
    ...
    for time_str in [
        ...
        result = next_occurrence(time_str, now=now)
