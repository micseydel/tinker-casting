...
class WokeProcessCompanion(WokeNote):
    ...
    def on_start(self):
        super().on_start()
        self.my_note.set_contents(NOTE_CONTENTS)
...
def run():
    ...
    woke_note = WokeProcessCompanion.wake(NOTE_NAME, main_conn)

    worker = Process(target=worker_proc, args=(worker_conn, Worker))
    worker.start()
